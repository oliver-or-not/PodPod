...
struct ListStyleBodyView_ghost: View {
    var key: PageKey
    ...
    init(key: PageKey, pageData: PageData, focusedIndex: Int? = nil, discreteScrollMark: Int? = nil, rowCount: Int? = nil, repeatState: RepeatState, alwaysShuffle: Bool, vibeIsActivated: Bool, videoZoomMode: VideoZoomMode, videoAutoplayMode: VideoAutoplayMode, libraryUpdateSymbolState: LibraryUpdateSymbolState, mainMenuBoolArray: [Bool])  {
        self.key = key
                            ...
                            HStack(spacing: 0) {
                                VStack(spacing: 0) {
                                    ForEach(min(discreteScrollMark, rowDataArray.count)..<min(discreteScrollMark + DesignSystem.Soft.Dimension.rangeOfRows, rowDataArray.count), id: \.self) { i in
                                        RowView_ghost(rowData: rowDataArray[i], indexInList: i, isFocused: i == focusedIndex, repeatState: repeatState, alwaysShuffle: alwaysShuffle, vibeIsActivated: vibeIsActivated, videoZoomMode: videoZoomMode, videoAutoplayMode: videoAutoplayMode, libraryUpdateSymbolState: libraryUpdateSymbolState, mainMenuBoolArray: mainMenuBoolArray)
                                    }
                                    Spacer()
                                        .frame(minHeight: 0)
                                }
...
struct ListStyleBodyView_ghost_Previews: PreviewProvider {
    
    static let pd = PageData(headerTitle: "iPod", pageBodyStyle: .list, rowDataArray: [
    ...
    static var previews: some View {
        ListStyleBodyView_ghost(key: .songs, pageData: pd, focusedIndex: 0, discreteScrollMark: 0, rowCount: pd.rowDataArray?.count, repeatState: .all, alwaysShuffle: true, vibeIsActivated: true, videoZoomMode: .fit, videoAutoplayMode: .off, libraryUpdateSymbolState: .done, mainMenuBoolArray: StatusModel.initialValueOfMainMenuBoolArray)
    }
}
