                            ...
                            HStack(spacing: 0) {
                                VStack(spacing: 0) {
                                    ForEach(min(discreteScrollMark, rowDataArray.count)..<min(discreteScrollMark + DesignSystem.Soft.Dimension.rangeOfRows, rowDataArray.count), id: \.self) { i in
                                        RowView_ghost(rowData: rowDataArray[i], indexInList: i, isFocused: i == focusedIndex, wantsToSeeTimeInHeader: wantsToSeeTimeInHeader, repeatState: repeatState, alwaysShuffle: alwaysShuffle, vibeIsActivated: vibeIsActivated, videoZoomMode: videoZoomMode, videoAutoplayMode: videoAutoplayMode, libraryUpdateSymbolState: libraryUpdateSymbolState, mainMenuBoolArray: mainMenuBoolArray)
                                    }
                                    Spacer()
                                        .frame(minHeight: 0)
                                }
